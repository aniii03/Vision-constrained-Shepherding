...
def run_simulation(N, D, e, alpha):
    ...
    pos_d = np.random.uniform(r, L - r, (1,2))
    ...
    pos_d_dat = np.full((steps, 2), np.nan)

    vel_d_dat = np.full((steps, 2), np.nan)

    spd_d_dat = np.full((steps, 1), np.nan)

    collect_t = np.full((steps, 1), np.nan)
    drive_t = np.full((steps, 1), np.nan)
    force_slow_t = np.full((steps, 1), np.nan)
    ...
    for t in range(1, steps):
        ...
        visible = np.zeros(len(active_idx))
        for i in range(len(active_idx)):
            j = active_idx[i]
            cos_ang = np.dot(vel_d_t_1,r_dg_shp[j]) / dist_rds[j]
            if cos_ang >np.cos(alpha / 2):
                visible[i] = 1

        r_dg_wall = pos_d_t_1 - boundary_b
        dist_dg_wall = row_norms(r_dg_wall)
        idx = np.where(dist_dg_wall < 3 * r)[0]
        ...
        if np.min(dist_rds) <= r:
            vel_next = vel_d_t_1 + 5 * rho_a * rwd_rep
            vel_next = vel_next / (np.linalg.norm(vel_next) + eps)
            pos_d = pos_d + 0.05 * vel_next
            ...
            visible_idx = active_idx[visible == 1]

            if len(visible_idx) == 0:
                theta_error = 2 * np.pi * np.random.rand()
                r_err = np.array([np.cos(theta_error), np.sin(theta_error)])

                vel_next = vel_d_t_1 + e * r_err + 5 * rho_a * rwd_rep
                vel_next = vel_next / (np.linalg.norm(vel_next) + eps)
            ...
            grp_centre /= len(visible_idx)

            active_pos = pos_s_t_1[visible_idx]
            ...
            if np.max(dist_gcm_i) >= f_n:
                ...
                s_p = visible_idx[s_local]
                d_behind = dist_gcm_i[s_local] + pc
                rc = d_behind * (r_gcm_i[s_local, :] / dist_gcm_i[s_local])
                rc = grp_centre + rc
                rdc = rc - pos_d_t_1
